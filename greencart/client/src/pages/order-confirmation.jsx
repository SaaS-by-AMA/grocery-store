...
const OrderConfirmation = () => {
  ...
  const computedSubtotal = order.items.reduce((acc, item) => {
    const unitPrice =
      (item.product && item.product.offerPrice) ?? item.price ?? 0;
    return acc + unitPrice * (item.quantity ?? 0);
  }, 0);

  const computedTax = computedSubtotal * TAX_RATE;
  const computedDelivery =
    computedSubtotal >= FREE_DELIVERY_THRESHOLD ? 0 : DELIVERY_CHARGE;
  const computedTotal = computedSubtotal + computedTax + computedDelivery;

  // order.amount is what the server saved; show both and reconcile if different
  const serverAmount = typeof order.amount === "number" ? order.amount : null;
  const amountsDiffer =
    serverAmount !== null && Math.abs(serverAmount - computedTotal) > 0.01;

  // Helper formatter
  const fmt = (n) =>
    `${currency}. ${Number.isFinite(n) ? n.toFixed(2) : "0.00"}`;
        ...
        <div className="border-t border-b border-gray-200 py-6 mb-6">
          ...
          <div className="space-y-4 mb-6">
            {order.items.map((item, index) => {
              ...
              const img = item.product?.image?.[0] ?? null;
                ...
                <div key={index} className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Unit: {fmt(unitPrice)}</p>
                    </div>
                  </div>
                  <p className="font-medium">{fmt(unitPrice * qty)}</p>
                </div>
              );
            })}
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between">
              ...
              <span>{fmt(computedDelivery)}</span>
            </div>
        ...
        <div className="mb-8">
          ...
          <div className="bg-gray-50 p-4 rounded-md">
            <p className="capitalize">
              <span className="font-medium">Method:</span> {order.paymentType ?? "COD"}
            </p>
            <p>
